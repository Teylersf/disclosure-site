/**
 * Upload disclosure-site/data/whitehouse-uap/ to Linode under whitehouse-uap/ prefix.
 * Skips files already present at correct size; never deletes.
 *
 *   npm run upload:whitehouse-uap
 */
...
const DATA = path.resolve(__dirname, "..", "data", "whitehouse-uap");
...
async function main() {
  ...
  console.log(`\n${DRY ? "DRY: would upload" : "Uploaded"}: ${uploaded}  Skipped: ${skipped}  Bytes: ${(bytes / 1024 / 1024).toFixed(2)} MB`);
}
